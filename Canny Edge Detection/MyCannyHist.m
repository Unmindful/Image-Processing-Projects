function [d4]=MyCannyHist(a,sigma,tau)
a=double(a);
tau1=tau;
tau2=0.4*tau;
...
for i=2:r-1
    for j=2:c-1
        if d3(i,j)==0
            if d1(i,j)>d1(i,j+1) && d1(i,j)>d1(i,j-1) && d1(i,j)<=tau1 && d1(i,j)>tau2 
                d5(i,j)=d1(i,j);
            end
        elseif d3(i,j)==45
            if d1(i,j)>d1(i-1,j+1) && d1(i,j)>d1(i+1,j-1) && d1(i,j)<=tau1 && d1(i,j)>tau2
                d5(i,j)=d1(i,j);
            end
        elseif d3(i,j)==90
            if d1(i,j)>d1(i+1,j) && d1(i,j)>d1(i-1,j) && d1(i,j)<=tau1 && d1(i,j)>tau2
                d5(i,j)=d1(i,j);
            end
        else
            if d1(i,j)>d1(i-1,j-1) && d1(i,j)>d1(i+1,j+1)&& d1(i,j)<=tau1 && d1(i,j)>tau2
...
for i=2:r-1
    for j=2:c-1
        if d5(i,j)>d4(i-1,j-1) && d5(i,j)>d4(i-1,j) && d5(i,j)>d4(i-1,j+1) && d5(i,j)>d4(i,j-1) && d5(i,j)>d4(i,j+1) && d5(i,j)>d4(i+1,j-1) && d5(i,j)>d4(i+1,j) && d5(i,j)>d4(i+1,j+1)
            d4(i,j)=d5(i,j);
        end
    end
end
